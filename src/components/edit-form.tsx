          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {(data.hostSide === 'groom' ? ['groom', 'bride'] : ['bride', 'groom']).map((side) => (
              <div key={side} className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="space-y-2">
                  <Label htmlFor={`${side}Father`}>{side === 'bride' ? 'वधूचे वडील' : 'वराचे वडील'}</Label>
                  <Input id={`${side}Father`} name={`${side}Father`} value={side === 'bride' ? data.brideFather : data.groomFather} onChange={handleChange} />
                </div>
                <div className="space-y-2">
                  <Label htmlFor={`${side}Mother`}>{side === 'bride' ? 'वधूची आई' : 'वराची आई'}</Label>
                  <Input id={`${side}Mother`} name={`${side}Mother`} value={side === 'bride' ? data.brideMother : data.groomMother} onChange={handleChange} />
                </div>
              </div>
            ))}
          </div>
